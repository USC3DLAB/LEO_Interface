...
class LEO(QMainWindow):
    ...
    def initUI(self):
        ...
        data_folder = QAction('*Import data sets for compare', self)
    ...
    def SD(self):
        ...
        QMessageBox.information(self, "Return",   'Solved the SP problem:\n  ' + self.smps_name + '\nResults are stored in ./projects/your prject/SP_output folder', QMessageBox.Ok )        
    ...
    def open_compared_file(self):
        ...
        if self.compared_data != []:
            ...
            for i in self.compared_data:
                j = j + i + '\n'
            QMessageBox.information(self, "Return",  'Imported the following data files for compare:\n'+j, QMessageBox.Ok )  

            if self.f != None:
                file = os.path.join('./projects/' + self.project, self.log_file_name)
                self.f = open(file,'a')
                self.f.write('Imported the following data files for compare:')
                self.f.write('\n')
                for i in self.compared_data:
                    self.f.write(i)
                    self.f.write('\n') 
            ...
            QMessageBox.information(self, "Return",  'Please data sets for compare', QMessageBox.Ok ) 
